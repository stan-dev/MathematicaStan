...
CmdStan::cmdStanDirectoryDoesNotExist="Modify CmdStanDirectory=`1`";
...
CmdStanDirectory::usage="Defines CmdStan directory";
...
(*CmdStanDirectory = "~/External_Software/cmdstan"; *)
CmdStanDirectory="/home/picaud/ExternalSoftware/cmdstan";
If[!DirectoryQ[CmdStanDirectory],
 Message[CmdStan::cmdStanDirectoryDoesNotExist, CmdStanDirectory]
 ];


SetCmdStanDirectory::usage="SetCmdStanDirectory[directory_String] modifies CmdStanDirectory";
SetCmdStanDirectory[directory_String]:=(Unprotect[CmdStanDirectory];CmdStanDirectory=directory;Protect[CmdStanDirectory];)


FileMultipleExtension[fileName_String]:=Module[{res},
...
CompileStanCode[stanCodeFileName_String, opts : OptionsPattern[]] :=
  Module[{command, stanExecFileName, tmpFile, verbose, runprocessResult},
   ...
   command = {"make","-C",CmdStanDirectory,stanExecFileName};
...
stanOptionToCommandLineString[opt_]:=Block[{optList,f,stack={}},
...
foldWhile[f_,test_,start_,secargs_List]:=Module[{last=start},Fold[If[test[##],last=f[##],Return[last,Fold]]&,start,secargs]];


stanOptionExistsQ[StanOptions[opt_Association],{keys__String}]:=Block[{status},foldWhile[Last[#1][#2]&,(status=KeyExistsQ[Last[#1],#2])&,{"",opt},{keys}];status]
...
setStanOption[StanOptions[org_Association], {keys__String}, value_] := Block[{tmp},
...
Block[{extracted,buffer,indices},
...
(* ::Subchapter:: *)
(*TODO definir method pour Stan Parallel*)
...
Block[{data,headerData,headerMeta,stringData,numericData,output},
...
Block[{extracted,index,values,dim,array},
