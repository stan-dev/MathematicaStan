...
provide this information when you want to modify option value. Options
organization are described in the [[http://mc-stan.org/interfaces/cmdstan][CmdStan user guide ("Command-Line
Options" section)]].

In *MathemeticaStan* you have *3* predefined option lists, you can print using:
...
or to remove a peculiar option:
#+BEGIN_SRC mathematica :exports code 
StanRemoveOptionOptimize["method.optimize.iter"]; 
#+END_SRC
...
Not yet implemented: remove option matching pattern, for instance:
#+BEGIN_SRC mathematica :exports code 
StanRemoveOptionOptimize["method.optimize.*"]; 
#+END_SRC

*** Output variable access and manipulation

See the *soft-k-means* example, involved functions are:
